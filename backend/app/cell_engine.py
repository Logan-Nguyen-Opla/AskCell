...
_SVD_SAMPLE = 10_000   # cells used to learn the 2-D subspace
_PROJ_BATCH  = 5_000   # cells projected per iteration (memory cap)


def _log_cpm_batch(raw: np.ndarray) -> np.ndarray:
    """Per-cell CPM + log1p normalisation on a dense (batch, genes) array."""
    row_sums = raw.sum(axis=1, keepdims=True)
    row_sums[row_sums == 0] = 1.0
    return np.log1p(raw / row_sums * 1e4)


def _read_dense(adata: ad.AnnData, idx: np.ndarray) -> np.ndarray:
    """Read a set of row indices from adata.X as a dense float64 array.

    Works for both in-memory and backed (HDF5) AnnData objects.
    h5py requires sorted indices for fancy indexing.
    """
    rows = adata.X[np.sort(idx), :]
    if sparse.issparse(rows):
        rows = rows.toarray()
    return np.asarray(rows, dtype=np.float64)


def _compute_2d_projection(adata: ad.AnnData) -> np.ndarray:
    """Return a (n_cells, 2) float32 array via truncated SVD on log-CPM data.

    For large datasets the SVD is learned on a random subsample of up to
    _SVD_SAMPLE cells; all cells are then projected in batches so memory
    usage stays bounded regardless of file size or backed/in-memory mode.
    """
    n_cells, n_genes = adata.n_obs, adata.n_vars
    k = min(2, n_cells - 1, n_genes - 1)
    if k < 1:
        return np.zeros((n_cells, 2), dtype=np.float32)

    # ---- Step 1: learn the 2-D subspace from a subsample ----
    rng = np.random.default_rng(42)
    sample_idx = (
        np.sort(rng.choice(n_cells, size=_SVD_SAMPLE, replace=False))
        if n_cells > _SVD_SAMPLE
        else np.arange(n_cells)
    )
    X_sample = _log_cpm_batch(_read_dense(adata, sample_idx))
    col_means = X_sample.mean(axis=0)          # gene-wise mean for centering
    X_centered = X_sample - col_means

    _, _, Vt = svds(X_centered, k=k)
    Vt = Vt[::-1]                              # descending variance order (k, genes)

    # ---- Step 2: project every cell through that subspace in batches ----
    coords = np.zeros((n_cells, 2), dtype=np.float32)
    for start in range(0, n_cells, _PROJ_BATCH):
        end = min(start + _PROJ_BATCH, n_cells)
        batch = _log_cpm_batch(_read_dense(adata, np.arange(start, end)))
        coords[start:end, :k] = ((batch - col_means) @ Vt.T).astype(np.float32)
...
class CellEngine:
    ...
    def __init__(self) -> None:
        ...
        self.umap_auto: bool = False  # True when X_umap was auto-computed (PCA)
    ...
    def load(self, file_path: str, filename: str, owns_file: bool = False) -> None:
        ...
        if "X_umap" not in adata.obsm:
            adata.obsm["X_umap"] = _compute_2d_projection(adata)
            self.umap_auto = True
        else:
            self.umap_auto = False
    ...
    # ------------------------------------------------------------------ #
    def compute_umap(self) -> dict:
        """Attempt to compute real UMAP. Returns a status dict.

        Tries umap-learn first (lighter, no scanpy required), then falls back
        to scanpy if available.  Returns ``{"success": True}`` on success, or
        ``{"success": False, "reason": ..., "script": ...}`` on failure.
        """
        self._assert_loaded()
        ...
        if self._backed:
            return {
                "success": False,
                "reason": "too_large",
                "script": self._umap_script(),
            }

        n_cells, n_genes = self.adata.n_obs, self.adata.n_vars

        # --- Try umap-learn (ships as pre-built wheel on Python ≤ 3.12) ---
        try:
            import umap as umap_lib  # umap-learn package

            X = _log_cpm_batch(_read_dense(self.adata, np.arange(n_cells)))
            col_means = X.mean(axis=0)
            X -= col_means

            k_pca = min(50, n_cells - 1, n_genes - 1)
            _, _, Vt = svds(X, k=k_pca)
            X_pca = (X @ Vt[::-1].T).astype(np.float32)

            reducer = umap_lib.UMAP(n_components=2, random_state=42, verbose=False)
            embedding = reducer.fit_transform(X_pca)
            self.adata.obsm["X_umap"] = np.asarray(embedding, dtype=np.float32)
            self.umap_auto = False
            return {"success": True}
        except ImportError:
            pass  # umap-learn not installed; try scanpy below
        ...
        try:
            ...
            sc.pp.log1p(tmp)
            sc.pp.pca(tmp, n_comps=min(50, tmp.n_vars - 1, tmp.n_obs - 1))
            sc.pp.neighbors(tmp)
            sc.tl.umap(tmp)
            self.adata.obsm["X_umap"] = np.asarray(tmp.obsm["X_umap"], dtype=np.float32)
            self.umap_auto = False
            del tmp
            return {"success": True}
        except ImportError:
    ...
    def _umap_script(self) -> str:
        fname = self.filename or "your_file.h5ad"
        out = fname.replace(".h5ad", "_umap.h5ad")
        return (
            f"import scanpy as sc\n"
            f"adata = sc.read_h5ad('{fname}')\n"
            f"sc.pp.normalize_total(adata, target_sum=1e4)\n"
            f"sc.pp.log1p(adata)\n"
            f"sc.pp.pca(adata)\n"
            f"sc.pp.neighbors(adata)\n"
            f"sc.tl.umap(adata)\n"
            f"adata.write_h5ad('{out}')"
        )

    def list_genes(self) -> list:
        """Return all gene symbols in the loaded dataset."""
        self._assert_loaded()
